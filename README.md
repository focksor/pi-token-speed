...
- 流式速度在**到达周期**上测量，并经由历史导出的可行域闸门过滤（见下）；响应完成（`message_end`）后定格为 **TTFT 之后全程的平均速度**（见「最终速度」），不再沿用流式估计器的尾值——长响应尾部减速时，定格的是整体平均而非尾段低速（闸门在历史不足 12 样本时不启用）。
...
响应完成（`message_end`）时 footer 定格的速度不是流式估计器的最后一个尾值，而是 **TTFT 之后全程的平均速度**：

```
最终速度 = (总输出 token − 首次到达携带的 token) / (完成时刻 − 首个 token 时刻)
```

- 分子排除首个到达的批次：那些 token 在 TTFT 窗口内就已生成，计入“TTFT 之后”的速度会让粗粒度交付（每 700ms+ 一个肥 delta 的网关）白得一整块的额度。排除后该值就是响应自身各到达周期速率的时长加权均值。
- 分母是首个 token 到完成之间的全部真实时间（墙钟）。这是“这个响应到底多快”的最诚实读数：长响应尾部减速/停滞时，流式估计器跟着尾段周期走（可能低到 20 tok/s），而定格值是整体平均——**收尾时最后停滞的数字不再受尾段拖累**。
- 流式过程中仍显示周期估计器的实时值（跟随局部节奏），只有完成时刻才切换为全程平均；要求响应至少有两个带 token 的到达，单个 delta 的短回复没有可测的 TTFT 后窗口，沿用原 `usage/elapsed` 回退（有历史时夹紧）。
- 可行域保护：平均值超出闸门上界（主要是冷启动肥 chunk 响应）时不显示上界数值，而是回退到鲁棒的流式估计（即用户刚才一直在读的值）；无历史时不夹紧，均值如实包含肥批次。
- subagent 聚合中“停止流式后 3 秒内保留的最后速度”同样更新为该最终值，与主会话段定格的数字一致。
