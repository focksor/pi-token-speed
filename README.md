...
2. 取最近 8 个周期速率。**窗口不足 7 个时取最小值**，7 个及以上取中位数。窗口小时必须保守：中位数只在离群值占少数时才有效，3 个周期里 2 个被污染就能带跑它（实测 2600 tok/s，而真实是 100）；而在早期 8 周期窗口里 3 个污染周期就能把中位数带到 10100。取最小值是数据稀缺时唯一诚实的读数，代价是窗口填满期间可能读**偏低**、绝不会读**偏高**。
...
- 无历史时若窗口内污染占多数（8 个周期中 ≥4 个），中位数仍可能被带跑；这与旧实现是同一个极限，闸门在 3 个响应后关闭该缺口。
